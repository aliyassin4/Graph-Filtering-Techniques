...
def doubly_stochastic(table, undirected = False, return_self_loops = False):
   ...
   table2 = table.copy()
   original_nodes = len(set(table["source"]) | set(table["target"]))
   table = pd.pivot_table(table, values = "nij", index = "source", columns = "target", aggfunc = "sum", fill_value = 0) + .001
   ...
   if undirected:
      ...
      while nx.number_weakly_connected_components(G) != 1 or nx.number_of_nodes(G) < original_nodes:
         ...
         i += 1
#    table1 = pd.melt(nx.to_pandas_adjacency(G).reset_index(), id_vars = "index")
   table = table[table["value"] >= 0]
   table.rename(columns = {"index": "source", "variable": "target", "value": "cij"}, inplace = True)
   table["score"] = table["cij"]
   table = table.merge(table2[["source", "target", "nij"]], on = ["source", "target"])
   if not return_self_loops:
      table = table[table["source"] != table["target"]]
   if undirected:
      table = table[table["source"] <= table["target"]]
   table = table.fillna(False)
   return table[["source", "target", "nij", "score", "ds_backbone"]]